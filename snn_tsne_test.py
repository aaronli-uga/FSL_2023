...
LastEditTime: 2023-06-01 14:18:04
...
X = np.load('dataset/8cases_jinan/test_set/X_test.npy')
y = np.load('dataset/8cases_jinan/test_set/y_test.npy')
...
trained_model = "saved_models/snn/new_2_loss_2d_snn_margin2_8cases_epochs30_lr_0.001_bs_128_best_model.pth"
...
with torch.no_grad():
    ...
    pred, _, _, _ = model(query_0, query_0)
    ...
    pred, _, _, _ = model(query_1, query_1)
    ...
    pred, _, _, _ = model(query_2, query_2)
    ...
    pred, _, _, _ = model(query_3, query_3)
    ...
    pred, _, _, _ = model(query_4, query_4)
    ...
    pred, _, _, _ = model(query_5, query_5)
    ...
    pred, _, _, _ = model(query_6, query_6)
    ...
    pred, _, _, _ = model(query_7, query_7)
...
# scatter_result_df = pd.DataFrame({'feature_1': X_embedding[:,0], 'feature_2': X_embedding[:,1], 'label': y_embedding})
# fig, ax = plt.subplots(1, figsize=(15,10))
# sns.scatterplot(x='feature_1', y='feature_2', hue='label', data=scatter_result_df, ax=ax,s=120)
# # lim = (scatter_result_df.min()-5, scatter_result_df.max()+5)
# # ax.set_xlim(lim)
# # ax.set_ylim(lim)
# ax.set_aspect('equal')
